...
<p align="center">
  <strong>MCP-сервер eBay — дайте Claude, Cursor и любому ИИ-ассистенту полный доступ к Sell API eBay. 313 инструмента для инвентаря, заказов, маркетинга и аналитики, работающих локально с вашими собственными ключами.</strong>
</p>
...
<p align="center">
  <img src="https://img.shields.io/badge/tools-313-8957e5?logo=ebay&logoColor=white" alt="313 инструмента API eBay" />
...
**eBay MCP** — это локальный сервер [Model Context Protocol](https://modelcontextprotocol.io), который подключает ИИ-ассистентов —Claude Desktop, Claude Code, Cursor, Cline, Windsurf, Zed, Continue.dev, Roo Code и Amazon Q— напрямую к **Sell API eBay**. Он предоставляет **313 инструмента**, охватывающих **100% поверхности Sell API eBay** (283 уникальных эндпоинта): управление инвентарём, обработка заказов, маркетинг продвигаемых объявлений, аналитика и инструменты разработчика. Всё работает на вашей машине через STDIO или локальный HTTP — **без облачного ретранслятора**, и ваши учётные данные eBay никогда не покидают компьютер.
...
- **313 инструмента API eBay** — 100% покрытие Sell API eBay: инвентарь, заказы, маркетинг, аналитика, метаданные, таксономия и инструменты разработчика.
...
- **Типобезопасность** — TypeScript от начала до конца, входные данные инструментов проверяются Effect, типы генерируются из OpenAPI.
...
| Валидация входных данных | Схемы на основе Effect + типы TypeScript на каждом инструменте | Нет — вы проверяете свои полезные данные сами |
...
| Покрытие API | 313 инструмента на 100% Sell API, готовы к вызову | Вы строите каждый запрос по документации |
...
**313 инструмента**, 100% покрытие Sell API, сгруппированы по категориям. Каждая ссылка ведёт к определениям инструментов и их обработчикам в [`src/tools/categories/`](src/tools/categories/):
...
| [Inventory](src/tools/categories/inventory.ts) | Товары инвентаря, предложения, локации, группы товаров, массовые операции, сопоставление SKU/локации, загрузка локальных фото и видео ([`media.ts`](src/tools/categories/media.ts), Media API) |
...
| [Trading (устаревший XML)](src/tools/categories/trading.ts) | Создание, изменение, повторное размещение и завершение объявлений с фиксированной ценой и аукционов |
...
Локальный сервер [Model Context Protocol](https://modelcontextprotocol.io), предоставляющий **313 инструмента**, охватывающих **100% Sell API eBay** (283 эндпоинта), ИИ-ассистентам — инвентарь, обработка заказов, маркетинг, аналитика и инструменты разработчика.
...
313 инструмента на 283 уникальных эндпоинтах — 100% Sell API eBay.
...
Вы взаимодействуете на естественном языке через ИИ-ассистента. Управление токенами OAuth, автоматические повторы с задержкой и типобезопасная валидация на основе Effect встроены. См. [таблицу сравнения](#ebay-mcp-против-обычного-api-ebay) выше.

### Может ли он загружать мои фото и видео?

Да. `ebay_upload_images`, `ebay_upload_video` и `ebay_attach_media_to_inventory_item` читают локальные файлы (абсолютные пути или ссылки `media://`) и загружают их через Media API eBay, возвращая URL изображений EPS и идентификаторы видео для `product.imageUrls` / `product.videoIds`. Доступ к файловой системе включается явно: пока `EBAY_MCP_MEDIA_DIRS` или `EBAY_MCP_MEDIA_ROOT` не укажут каталоги, прочитать ничего нельзя. См. [Photos and videos from local files](README.md#photos-and-videos-from-local-files) (на английском).

### Поддерживает ли он аукционы?

Да, через инструменты предложений Inventory API (REST): создайте предложение с `format: "AUCTION"`, `auctionStartPrice`, необязательным `auctionReservePrice` и `listingDuration` в днях, затем опубликуйте его. См. [Auction offers](README.md#auction-offers) (на английском). Инструменты Trading API используют тот же переключатель: `ebay_create_listing` с `format: "AUCTION"` отправляет `AddItem` с `ListingType` Chinese, стартовой ставкой `StartPrice` и `ListingDuration` в днях.

### Поддерживает ли он устаревший Trading API eBay (XML)?

Да. Операции создания, изменения, повторного размещения и завершения объявлений поддерживаются через инструменты Trading API — как для объявлений с фиксированной ценой (семейство `AddFixedPriceItem`, по умолчанию), так и для аукционов (`format: "AUCTION"` → `AddItem`, `ReviseItem`, `EndItem`, `RelistItem`).
...
TypeScript и Node.js (ESM), с официальным MCP SDK, валидацией на основе Effect с Zod-совместимым MCP-адаптером и типами, сгенерированными из OpenAPI.
