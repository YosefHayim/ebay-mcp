...
EBAY_MCP_UI=on                      # 交互式视图；设为 "off" 可强制返回纯 JSON
```
...
在支持 [MCP Apps](https://modelcontextprotocol.io) 的宿主上，常用的读取类工具会将结果渲染为交互式视图，而非原始 JSON —— 可排序的**表格**、详情**卡片**或**图表** —— 并采用宿主自身的主题。在其他任何环境中，同样的这些工具都会返回纯 JSON，因此不会出现任何问题。
