...
| `tools/`         | MCP tool wiring — `registry.ts`, `contracts.ts`, `schemas.ts`, `defineTool.ts`, and `categories/` (family files co-locate each tool definition with its handler via `defineTool`; marketing pairs a large definition catalog with a handler map and must keep one public definition per handler). `tools/trading/` holds handler-side input resolvers that do MCP-boundary I/O the API layer must not (e.g. `uploadImageInput.ts` reads a local file / decodes base64 into bytes before the Trading client sees them) |
